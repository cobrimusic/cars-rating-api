		...
		try {
			...
			if (created)
				return this.returnCreated(
					res,
					created,
					`${this.service.modelName} creado satisfactoriamente!`
				);

			return this.returnBadRequest(
				res,
				`Error ${this.service.modelName}`
			);
		} catch (err) {
			return this.processException(res, err);
		}
		...
		try {
			if (await this.service.update(req.body, id))
				return this.return(
					...
					`${this.service.modelName} actualizado!`
				);

			return this.returnServerError(
				res,
				`Error durante la actualización ${this.service.modelName}.`
			);
		} catch (err) {
			return this.processException(res, err);
		}
